...
class Dashboard extends Component {
    ...
    getMeetings() {
        ...
        firebase.database().ref(`/meetings/`).on('child_added', snap => {
            ...
            if(meeting.senderId === firebase.auth().currentUser.uid){
            meetingSender.push(meeting)
            this.setState({ meetingSender })
        }
        if(meeting.recieverId === firebase.auth().currentUser.uid){
                                    ...
                                    if (value.recieverId == firebase.auth().currentUser.uid) {
                                        ...
                                        return <TableRow>
                                            ...
                                            <TableCell>
                                                ...
                                                <Button color='primary' onClick={this.acceptReq.bind(this, value.key, index)}>Accept</Button>/<Button color='secondary'>Reject</Button></div> : value.status} 
                                                </TableCell>
                                            <TableCell>{value.date} & {value.time}</TableCell>
                                            <TableCell><Button color='secondary'>Get Directiom</Button></TableCell>
                                        </TableRow>
